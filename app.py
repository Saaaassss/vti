...
def extract_favicon(url):
    ...
    try:
        ...
        icons = soup.find_all("link", rel=True)
        for link in icons:
            rel = link.get("rel")
            href = link.get("href")
            ...
            rel = " ".join(rel).lower() if isinstance(rel, list) else rel.lower()
 
            if "icon" in rel:
                return urljoin(base, href)
 
    except Exception as e:
        print("Favicon HTML error:", e)
...
def get_favicon():
    ...
    print(f"[FAVICON] Requested favicon for URL: {url}")
    if not url:
        print("[FAVICON] No URL provided.")
        return jsonify({'favicon': None, 'status': 'error'}), 400
    icon = extract_favicon(url)
    print(f"[FAVICON] Extracted favicon: {icon}")
    return jsonify({
        'favicon': icon,
...
def proxy_favicon():
    ...
    print(f"[PROXY-FAVICON] Proxying favicon for URL: {img_url}")
    if not img_url:
        print("[PROXY-FAVICON] No URL provided.")
    ...
    try:
        ...
        r.raise_for_status()
        print(f"[PROXY-FAVICON] Successfully fetched favicon: {img_url}")
        return Response(r.content, mimetype=r.headers.get('Content-Type', 'image/png'))
    except Exception as e:
        print(f"[PROXY-FAVICON] Proxy error: {e}")
